...
describe('core/HTTPTransport', () => {
    it('should send requests', async () => {
        await http.request('http://mockhost/mock/path', {method: METHODS.GET})
            .then(({status}) => {
    ...
    it('should send requests with headers', async () => {
       await http.request('http://mockhost/mock/path', {headers: {'mock-header': 'test'}})
           .then((response) => {
    ...
    it('should send get requests with parameters', async () => {
        const response = await http.get('http://mockhost/mock/path', {data: {param: 'mock'}})
    ...
    it('should send post requests with JSON', async () => {
        await http.post('http://mockhost/mock/path', {data: {postprop: 'mock'}})
            .then(({status, response}) => {
                expect(status).toBe(200);
    ...
    it('should send put requests with JSON', async () => {
        await http.put('http://mockhost/mock/path', {data: {putprop: 'mock'}})
            .then(({status, response}) => {
                expect(status).toBe(200);
    ...
    it('should send delete requests with JSON', async () => {
        await http.delete('http://mockhost/mock/path', {data: {deleteprop: 'mock'}})
            .then(({status, response}) => {
                expect(status).toBe(200);
